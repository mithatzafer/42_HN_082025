...
char	**ft_initialise_row_dict(int grid_size)
{
	...
	{
		permutation_dict_pt[i] = malloc(grid_size);
		j = 0;
		while (j < grid_size)
		{
			permutation_dict_pt[i][j] = j + 1;
			j++;
		}
		i++;
	}
...
void	ft_generate_row_permutations(int grid_size, char **permutation_dict_pt)
{
	...
	while (i < grid_size)
	{
		...
		permutation_dict_pt[i][0] = row_to_work_on[i];
		i++;
	}
	i = 0;
	while (i < total_permutations - 1)
	{
		...
		while (j < grid_size)
		{
			permutation_dict_pt[j][i] = row_to_work_on[j];
			j++;
		}
	}
}
